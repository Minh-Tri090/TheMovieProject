...
import { getMovieById, getComments, addComment, updateComment, addReply, updateReply, logHistory } from '../services/api';
...
export default function MovieDetail({ isWatchMode = false }) {
  ...
  // Tự động ghi lịch sử xem khi user bắt đầu xem
  useEffect(() => {
    if (isWatching && user && movie) {
      const isMongoId = /^[a-f\d]{24}$/i.test(String(id));
      logHistory({
        movieId: isMongoId ? id : null,
        tmdbId: isMongoId ? null : id,
        movieTitle: movie.title,
        moviePoster: movie.poster,
        progress: 5, // Đánh dấu "đang xem"
      });
    }
  }, [isWatching, user, movie]);
  ...
  const handleVote = async (itemId, type) => {
    ...
    for (const c of comments) {
      ...
      if (c.replies && c.replies.some(r => r.id === itemId)) {
        targetItem = c.replies.find(r => r.id === itemId);
        isReply = true;
    ...
    let newUserVote = targetItem.userVote || 0;
    let newUpvotes = targetItem.upvotes || 0;
    let newDownvotes = targetItem.downvotes || 0;
    ...
    setComments(prev => prev.map(c => {
      if (!isReply && c.id === itemId) return { ...c, ...updatedFields };
      if (isReply && c.id === parentCommentId) {
        return { ...c, replies: c.replies.map(r => r.id === itemId ? { ...r, ...updatedFields } : r) };
      }
      return c;
    }));
    ...
    try {
      if (isReply) {
        await updateReply(parentCommentId, itemId, { upvotes: updatedFields.upvotes, downvotes: updatedFields.downvotes });
      } else {
        await updateComment(itemId, { upvotes: updatedFields.upvotes, downvotes: updatedFields.downvotes });
      }
    } catch (err) {
      console.error(err);
    }
  ...
  if (!movie) return (
    <div className="container page-section" style={{ textAlign: 'center', padding: '80px 20px' }}>
      ...
      <p style={{ color: '#94a3b8', fontSize: '1.1rem', marginBottom: '24px' }}>
        Không tìm thấy thông tin phim này.
      </p>
      <button
        onClick={() => navigate('/')}
        style={{ padding: '10px 24px', background: '#f5c518', color: '#000', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.95rem' }}
      >
        ← Về trang chủ
      </button>
    </div>
        ...
        <section className="surface detail-surface">
          ...
          <div className="page-section comments-section">
            ...
            <div
              ...
              style={{ opacity: user ? 1 : 0.6, pointerEvents: user ? 'auto' : 'none' }}
            >
              ...
              <div className="comment-footer">
                <label className="spoiler-toggle">
                  <div
                    ...
                    className={`spoiler-switch ${isSpoiler ? 'on' : ''}`}
                    role="switch"
                    aria-checked={isSpoiler}
                  >
                    <div className="spoiler-switch-thumb" />
                  </div>
                  Tiết lộ?
                </label>
                <button
                  ...
                  onClick={async () => {
                    const newCommentData = {
                      movieId: id,
                      user: user?.name || user?.username || user?.email || "Người dùng",
                ...
                {comments.map((c) => (
                  <div key={c.id} className="comment-card">
                    ...
                    <div className="comment-body">
                      ...
                      {replyingTo === c.id && (
                        <div className="comment-input-box" style={{ marginTop: '16px' }}>
                          <div style={{ position: 'relative' }}>
                            <textarea
                              className="comment-textarea comment-textarea--reply"
                              placeholder="Viết bình luận"
                              value={replyText}
                          ...
                          <div className="comment-footer comment-footer--reply">
                            <label className="spoiler-toggle">
                              <div
                                onClick={() => setReplyIsSpoiler(!replyIsSpoiler)}
                                className={`spoiler-switch ${replyIsSpoiler ? 'on' : ''}`}
                              >
                                <div className="spoiler-switch-thumb" />
                              </div>
                              Tiết lộ?
                            ...
                            <button
                              ...
                              onClick={async () => { 
                                ...
                                const savedReply = await addReply(c.id, newReplyData);
                                if (savedReply) {
                                  setComments(prev => prev.map(cmt => cmt.id === c.id ? { ...cmt, replies: [...(cmt.replies || []), savedReply] } : cmt));
                        ...
                        <div className="replies-list">
                          {c.replies.map((r) => (
                            <div key={r.id} className="reply-card">
                              <img src={r.avatar} alt="avatar" className="reply-avatar" />
                              <div className="comment-body">
